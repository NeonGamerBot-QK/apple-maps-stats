...
export const parseStr = (content: string) => {
    ...
    if (startANDEnd(content.trim(), "I will arrive at", "running late.")) {
      ...
      result.timestamp = content.split("around ")[1].split(" I")[0].replaceAll(".", "");
    } 
    else if (startANDEnd(content.trim(), "My updated arrival time to", "is now around")) {
      result.type = Phrases.UPDATED;
      result.location = content.split("My updated arrival time to ")[1].split(" is now around")[0];
      result.timestamp = content.split("is now around ")[1].split(".")[0];
    } else if (content.includes("arriving at")) {
      result.type = Phrases.ENDING;
      result.location = content.split("arriving at ")[1].split(".")[0].split(" soon")[0];
    }
    return result;
